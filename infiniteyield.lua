...
local Plugins = {table.insert(Plugins, "/storage/emulated/0/Download/meu-plugin.lua")
}
...
task.spawn(function()
    ...
    local success, err = pcall(function()
        loadstring(readfile(plugin))()
    end)
    if not success then
        warn("Erro ao carregar plugin:", err)
    end
end
-- Código original do Infinite Yield
-- ... (código do Infinite Yield aqui)

--// Agora cole o código do seu plugin aqui:
serverlocked = false
bannedusers = {}
...
-- ... (restante do código do plugin aqui)
